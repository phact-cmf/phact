...
class Adapter
{
    ...
    protected function buildCriteria($statements, $bindValues = true)
    {
        ...
        foreach ($statements as $statement) {
            ...
            if (is_array($key)) {
                list($nestedSql, $nestedBindings) = $this->buildCriteria($key);
                $bindings = array_merge($bindings, $nestedBindings);
                $criteria .= $statement['joiner'] . ' (' . $nestedSql . ') ';
    ...
    protected function buildJoin($statements)
    {
        ...
        foreach ($statements['joins'] as $joinArr) {
            ...
            $condition = '';
            if ($joinArr['key'] instanceof Raw) {
                $condition = (string) $joinArr['key'];
            } elseif (isset($joinArr['value']) && isset($joinArr['operator'])) {
                $condition = $this->wrapSanitizer($joinArr['key']) . ' ' . $joinArr['operator'] . ' ' . $this->wrapSanitizer($joinArr['value']);
            }
            $sqlArr = array(
                $sql,
